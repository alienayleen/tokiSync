# ⚡️ TokiSync (토끼싱크) v1.20.0

**북토끼, 뉴토끼, 마나토끼**의 콘텐츠를 **구글 드라이브로 직접 업로드**하고, **전용 웹 뷰어**를 통해 편리하게 관리/열람할 수 있는 올인원 솔루션입니다.

> **🚀 v1.20.0 업데이트 요약:**
> - **신형 복호화 엔진 및 다형성 팝업 IPC 수집 체계**: 토큰 기반 XOR 디코딩 분석 엔진을 도입하고, 봇 감지 우회를 위해 Controller-Worker IPC 통신 기반 팝업 수집 모델을 도입하여 보안 무력화 및 소설 문단 유실을 철저하게 차단합니다.
> - **스크롤 뷰어 이미지 왜곡(Layout Shift) 해소**: 뷰어 스크롤 모드에서 플레이스홀더를 고정하기 위한 `minHeight` 제약이 로딩 후에도 스태틱하게 유지되어 이미지가 찌그러지는 현상을 완전히 해결했습니다. 로드 완료 즉시 `minHeight`를 `auto`로 전환해 원본 크기와 비율을 정상 노출시킵니다.

---

## ✨ 주요 기능

### 📥 수집기 (UserScript) - v1.20.0
...
### 📊 뷰어 2.0 (Cinematic & Refined) - v1.20.0
