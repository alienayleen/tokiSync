# ⚡️ TokiSync (토끼싱크) v1.25.0

**북토끼, 뉴토끼, 마나토끼**의 콘텐츠를 **구글 드라이브로 직접 업로드**하고, **전용 웹 뷰어**를 통해 편리하게 관리/열람할 수 있는 올인원 솔루션입니다.

> **🚀 v1.25.0 업데이트 요약:**
> - **파서 규칙 스토리지 단일 통합**: 간편 에디터(Form Editor)와 트리뷰 에디터(Tree Editor)가 사용하는 저장 대상을 `TOKI_PARSER_RULES`로 일원화하여 설정 정합성을 극대화했습니다.
> - **내장 샘플 룰 자동 주입 메커니즘**: 백그라운드 원격 다운로드 대신 빌트인 샘플 규칙(`toki_common`)을 동적으로 즉각 주입하여 초기 실행 시 파싱 오류를 원천 차단했습니다.
> - **워커(Worker) 중복 실행 방어 가드**: iframe 내에서 워커 스크립트가 과다하게 중복 주입되어 비정상 수집을 유발하던 버그를 해결하기 위해 중복 가드 및 전역 IPC 리스너 레지스트리 가드를 도입했습니다.

---

## ✨ 주요 기능

### 📥 수집기 (UserScript) - v1.25.0
