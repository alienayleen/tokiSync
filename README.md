# ⚡️ TokiSync (토끼싱크) v1.9.1

**북토끼, 뉴토끼, 마나토끼**의 콘텐츠를 **구글 드라이브로 직접 업로드**하고, **전용 웹 뷰어**를 통해 편리하게 관리/열람할 수 있는 올인원 솔루션입니다.

> **🚀 v1.9.1 업데이트 요약:**
> **UI 아키텍처 현대화 (Glassmorphism)**: 뷰어 V2의 프리미엄 감성을 수집기 코어에 이식했습니다. **Glassmorphism 기반 디자인 시스템**을 전면 적용하여 시각적 일관성을 확보하고, **Direct Google Drive API (v3)** 기반의 고속 이력 동기화를 통해 반응성을 대폭 개선했습니다. 또한 **Tree Rule Editor**의 시각적 피드백을 강화하여 파싱 규칙 관리가 더욱 직관적으로 변했습니다.

---

## ✨ 주요 기능

### 📥 수집기 (UserScript) - v1.9.1

- **✨ Glassmorphism UI (New)**: 블러(Blur) 효과와 반투명 레이아웃을 적용한 프리미엄 디자인 시스템.
- **🔄 Direct Sync Engine (New)**: GAS를 거치지 않는 Direct Drive API v3 활용으로 이력 동기화 속도 3배 향상.
- **✅ 시각적 다운로드 표시 (New)**: 다운로드 완료 항목에 클래스 기반의 체크마크 및 오버레이 효과 적용.
- **🧩 Tree Rule Editor (Advanced)**: JSON 트리 구조로 파싱 규칙을 관리하고 즉시 테스트 가능한 전용 인터페이스.
...
### 📊 뷰어 2.0 (Cinematic & Refined) - v1.9.1
