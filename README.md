# ⚡️ TokiSync (토끼싱크) v1.8.3

**북토끼, 뉴토끼, 마나토끼**의 콘텐츠를 **구글 드라이브로 직접 업로드**하고, **전용 웹 뷰어**를 통해 편리하게 관리/열람할 수 있는 올인원 솔루션입니다.

> **🚀 v1.8.3 업데이트 요약:**
> **UI/UX 전면 개편 및 소설 엔진 고도화**: 아코디언 메뉴를 탈피한 **상단 탭 시스템**과 **520px 확장 레이아웃**으로 조작 편의성을 극대화했습니다. 소설 수집 시 **TXT 포맷**을 추가 지원하며, API 복호화 과정의 불필요한 데이터를 정밀 제거하여 가독성을 높였습니다. 또한, 사용자 설정에 따른 **다운로드 속도 자율화**를 실현하고 실패 리포트 자동 생성 기능을 탑재하여 안정성을 보강했습니다.

---

## ✨ 주요 기능

### 📥 수집기 (UserScript) - v1.8.3

- **📱 모던 탭 UI (New)**: 다운로드/설정/시스템 기능을 탭으로 분리하고 520px 너비로 확장하여 쾌적한 환경 제공.
- **⚙️ 소설 출력 포맷 선택 (New)**: EPUB 표준 포맷 외에도 일반 텍스트(TXT) 저장을 선택 가능.
- **📄 다운로드 실패 리포트 (New)**: 네트워크 오류 등으로 인한 누락 발생 시 상세 내역이 담긴 리포트(.txt) 자동 생성.
- **🛡️ 정밀 복호화 엔진**: 암호화된 소설 데이터를 API를 통해 완벽히 복호화하며, 불필요한 메타데이터를 정밀 제거.
- **🚀 속도 정책 자율화**: 소설 복호화 시에도 사용자가 설정한 다운로드 속도 정책을 100% 준수.
...
### 📊 뷰어 2.0 (Cinematic & Refined) - v1.8.3
