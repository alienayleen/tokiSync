...
> **성능 및 캐시 엔진 혁신**: IndexedDB 영구 바이너리 캐시(L2) 도입, 6분할 터보 다운로드 엔진 탑재, 가변 병렬도(Threads 1-3) 설정 추가으로 저속 네트워크(4G) 대응력 강화. 히스토리 조회 페일세이프 및 사전 다운로드(Preload) 안정화.

- **[Core]** **히스토리 조회 페일세이프**: 전체 히스토리 조회 타임아웃 발생 시, 개별 에피소드 단위로 드라이브를 확인하는 2차 검증 로직 도입.
- **[Parser]** **썸네일 추출 로직 최적화**: 웹툰/소설 등 다양한 페이지 구조에 대응하기 위한 Fallback 셀렉터 추가 및 파싱 성공률 향상.
- **[Viewer]** **영구 바이너리 캐시 (L2)**: IndexedDB를 활용해 에피소드 데이터를 영구 저장. 새로고침 후 재다운로드 없이 광속 로딩.
- **[Viewer]** **6분할 병렬 다운로드 엔진**: 모든 에피소드를 균등하게 6개 청크로 분할하여 받는 6-Layer 엔진 탑재 및 바이트 정합성 보정.
- **[Viewer]** **백그라운드 사전 다운로드(Preload)**: 현재 회차 열람 중 다음 회차를 미리 다운로드하여 끊김 없는 독서 경험 제공.
- **[Viewer]** **가변 병렬도 설정**: 저속 네트워크(4G) 대응을 위해 1~3개의 다운로드 스레드 선택 옵션 추가.
- **[UI]** 뷰어 설정 화면에 `Download Threads` 선택 옵션 및 `Background Preload` 토글 추가.
