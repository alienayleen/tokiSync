...
v1.9.1부터는 두 가지 방식으로 파싱 규칙을 관리할 수 있습니다:

### 4.1 Tree Rule Editor (권장)
1. TokiSync 메뉴 → **[파서 관리]** 탭 클릭
2. 시각적인 트리 구조에서 사이트별 규칙을 직접 수정/추가
3. **[규칙 적용]** 버튼을 클릭하여 즉시 반영

### 4.2 수동 JSON 설정
1. TokiSync 메뉴 → **[설정]** 탭 → **고급 설정**
2. **커스텀 파싱 룰 (JSON Array)** 칸에 JSON 직접 편집
3. **저장** 클릭
